...
const warn = (msg) => console.log(`⚠ ${msg}`); // شذوذ في البيانات تعالجه الصفحة بأمان — لا يوقف النشر
...
if (!jsonMatch) fail('كتلة بيانات المحادثة #rw-data غير موجودة');
  ...
  if (cd) {
    ...
    // كل خيار يعرضه المرشد يحمل وصفًا بشريًا — هذا جوهر إعادة الصياغة في v4
    const noHint = [];
    for (const [tag, p] of Object.entries(cd.periods || {})) {
      if (!p.hint) noHint.push(`فترة ${tag}`);
      for (const k of p.packs || []) if (!k.hint) noHint.push(k.label);
    }
    for (const d of cd.durations || []) if (!d[3]) noHint.push(d[1]);
    if (!cd.hourHint) noHint.push(cd.hourLabel);
    check(noHint.length === 0, `كل خيار في المحادثة يحمل وصفًا بشريًا${noHint.length ? ` — ينقص: ${noHint.join('، ')}` : ''}`);

    // بيانات التوصية والمقارنة موجودة وسليمة
    const recOk = cd.guide && cd.periods?.m?.packs?.some((k) => k.hours === cd.guide.recommendedHours);
    check(!!recOk, 'ساعات التوصية في guide تطابق باقة موجودة');
    check(!!(cd.compare?.week && cd.compare?.month && cd.compare?.term), 'جدول المقارنة (أسبوع/شهر/ترم) موجود');

    // التوفير يُحسب من الأسعار: نتأكد أنه موجب فعلًا في كل الباقات.
    // الصفحة تُخفي السطر إن لم يكن موجبًا، فالخلل هنا تنبيه لا فشل.
    if (cd.compare) {
      const odd = [];
      for (const [tag, p] of Object.entries(cd.periods || {}))
        for (const k of p.packs || [])
          for (const [durKey, cmp] of Object.entries(cd.compare)) {
            const diff = cmp.count * k.prices[cmp.per].price - k.prices[durKey].price;
            if (diff <= 0) odd.push(`${tag}/${k.label}/${durKey} = ${diff}`);
          }
      if (odd.length) warn(`التزام أطول لا يوفّر شيئًا في: ${odd.join('، ')} — لن يظهر سطر التوفير هناك`);
      else pass('الالتزام الأطول أوفر دائمًا (30 مقارنة) — سطر التوفير صادق');
    }
  }
}
...
const required = [
  ...
  ['كم يبقى طفلك عندنا في اليوم عادةً؟', 'سؤال الساعات (بلغة الأسرة)'],
  ['تمام. ولأي مدة تريد الاشتراك؟', 'سؤال المدة'],
  ['هذه أنسب باقة لما اخترته. اضغط «اشترك الآن» لإتمام الطلب:', 'رسالة النتيجة'],
  ...
  ['من 6:45 صباحاً', 'وصف الفترة الصباحية'],
  ['حتى 9 مساءً', 'وصف الفترة المسائية'],
  // v4: تراجع واقتراح وتعليل وخطوات
  ['رجوع ↩', 'زر التراجع'],
  ['لست متأكد؟', 'مخرج «لست متأكد»'],
  ['الخطوة ', 'مؤشر الخطوات'],
  [' من 3', 'مؤشر الخطوات (من 3)'],
  ['وهي الخيار الأوسط بين باقاتنا. أبدأ بها؟', 'اقتراح الساعات'],
  ['أرني بقية الخيارات', 'رفض اقتراح الساعات'],
  ['أختاره لك؟', 'اقتراح المدة'],
  ['أرني بقية المدد', 'رفض اقتراح المدة'],
  ['أوفر بـ', 'تعليل التوفير'],
  ['يوم دراسي كامل', 'وصف بشري لباقة 6 ساعات'],
  ['جرّب ', 'بديل قريب بعد النتيجة'],
